  ...
  loadStyle(presetName) {
    switch (presetName) {
      ...
      case "TerminalGreen":
        ...
        break;
      case "TerminalRed":
        this.style.TerminalRed();
        break;
      case "TerminalBlue":
        this.style.TerminalBlue();
        break;
      case "TerminalYellow":
        this.style.TerminalYellow();
        break;
      case "TerminalMagenta":
        this.style.TerminalMagenta();
        break;
      default:
...
class GuiObject {
  constructor(label, x, y, w, h) {
    ...
    this.w          = w;
    this.h          = h;
    this.int        = false;      // Used to have sliders lock to ints
...
class GuiSlider extends GuiObject {
  ...
  setActive(active) {
    ...
    if (this._active && this._selU != null) {
      ...
      this.val  = this.int ? int(this.val) : this.val;
    }
  }
  ...
  drawState(fillBg, fillTrack, fillHandle, strokeBg, strokeTrack, strokeHandle) {
    ...
    push();
      ...
      // Render track
      this.style.trackWidth = constrain(this.style.trackWidth, 0, 1);
      let h = this.h-20;
      ...
      push();
        ...
        if (this.style.trackWidth > 0) {
          rect(this.x+10, 
               this.y+10 + h*(1-this.style.trackWidth)*0.5, 
               xpos, 
               h*this.style.trackWidth, 
  ...
  drawState(fillBg, fillTrack, fillHandle, strokeBg, strokeTrack, strokeHandle) {
    ...
    push();
      ...
      push();
        ...
        if (this.style.trackWidth > 0) {
            rect(this.x+10 + w*(1-this.style.trackWidth)*0.5, 
                 this.y+ypos+10, 
                 w*this.style.trackWidth, 
                 this.h-ypos-20, 
      ...
      push();
        ...
        if (ypos >= halfYpos) {
          if (this.style.trackWidth > 0) {
            ...
            line(this.x+10 + w*(1-this.style.trackWidth)*0.5, 
                 this.y+halfYpos+8, 
                 this.x+10 + w*(1-this.style.trackWidth)*0.5, 
                 this.y+8+ypos);
          }
        }
        else {
...
class GuiSlider2d extends GuiObject {
  ...
  setActive(active) {
    ...
    if (this._active && this._selU != null && this._selV != null) {
      ...
      if (this.int && this._type == "slider2d") {
