...
class GuiContext {
  ...
  onMove(obj, p) {
    ...
    if (obj.onHold != null) {
      if (typeof obj.onHold === "function") {
        obj.onHold();
      }
      else {
        console.error("Please assign a valid function for " + obj.label + " \'onHold\' callback.");
      }
    }
  }
...
//    this.lock       = true;
  ...
  // Stores the object's state for reference in a future frame
  storePrev() {
    if (this.pressed) {
      this.held = true;
    }
